...
def make_line_plot(
    ...
    ax.set_title(title)
    save_figure(fig, fig_folder, file_name)
    plt.close(fig)
...
def main() -> None:
    ...
    if not excel_path.is_file():
        raise FileNotFoundError(f"The Excel file was not found: {excel_path}")
    ...
    ax.legend()
    save_figure(fig, fig_folder, "Fig06_SynchronizedSignals_Time")
    plt.close(fig)
    ...
    ax.legend()
    save_figure(fig, fig_folder, "Fig07_TriggerZoom_0_30ms")
    plt.close(fig)
    ...
    for ax in axes:
        ax.grid(True, alpha=0.3)
    save_figure(fig, fig_folder, "Fig15_Report_Data_Summary")
    plt.close(fig)
    ...
    for ax in axes:
        ax.grid(True, alpha=0.3)
    save_figure(fig, fig_folder, "Fig16_LSTM_Normalized_Sequences")
    plt.close(fig)
    ...
    save_figure(fig, fig_folder, "Fig17_LSTM_Model_Workflow")
    plt.close(fig)
    ...
    ax.legend()
    save_figure(fig, fig_folder, "Fig20_LSTM_Training_RMSE_History")
    plt.close(fig)
    ...
    ax.legend()
    save_figure(fig, fig_folder, "Fig21_LSTM_Training_Loss_History")
    plt.close(fig)
    ...
    ax.legend()
    save_figure(fig, fig_folder, "Fig11_LSTM_Displacement_Prediction")
    plt.close(fig)
    ...
    ax.legend()
    save_figure(fig, fig_folder, "Fig12_LSTM_Force_Prediction")
    plt.close(fig)
    ...
    save_figure(fig, fig_folder, "Fig13_LSTM_Prediction_Errors")
    plt.close(fig)
    ...
    save_figure(fig, fig_folder, "Fig14_LSTM_TriggerZoom")
    plt.close(fig)
    ...
    axes[1].legend()
    save_figure(fig, fig_folder, "Fig22_LSTM_Accuracy_Time_Response")
    plt.close(fig)
    ...
    save_figure(fig, fig_folder, "Fig23_LSTM_Accuracy_Parity_Plots")
    plt.close(fig)
    ...
    save_figure(fig, fig_folder, "Fig24_LSTM_Test_Accuracy_Metrics")
    plt.close(fig)
