...
Since only two chirp experiments are available, reserving one entire chirp for testing would make the problem too dependent on mismatch between the two experiments. Therefore, each chirp is divided into blocks, and each block is split into training, validation, and testing portions.

\begin{databox}{Data-splitting strategy}
Each chirp block is divided into 70\% training, 15\% validation, and 15\% testing. This keeps both chirps represented in all stages while ensuring that validation and test windows are separated from training updates. The validation data are used for model selection, and the test data are used only for final evaluation.
\end{databox}
