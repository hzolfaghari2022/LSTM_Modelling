...
# Extra features to avoid mean or straight-line predictions from one trajectory.
# Current is still the physical input. Time and current derivative are auxiliary
# sequence features that help the model learn where it is within this one COMSOL trajectory.
USE_TIME_FEATURE = True
USE_CURRENT_DERIVATIVE_FEATURE = True

# If True, the final model is fine tuned on all available windows after validation based
# model selection. This improves reconstruction of the one available COMSOL trajectory.
# It should not be interpreted as generalization to new experiments.
FINE_TUNE_ON_ALL_WINDOWS = True
...
BRANCH_NAME = "main"
TARGET_FOLDER_NAME = "ESA_Single_Actuator_LSTM_Model_Python_Final_V2"
...
def main() -> None:
    ...
    # ------------------------------------------------------------
    # Input features. Current remains the physical actuator input.
    # The normalized time and current derivative are optional auxiliary features
    # used to improve trajectory learning from the single available COMSOL run.
    input_features = [current_model.reshape(-1, 1)]

    if USE_TIME_FEATURE:
        time_feature = ((t - np.min(t)) / (np.max(t) - np.min(t))).reshape(-1, 1)
        input_features.append(time_feature)

    if USE_CURRENT_DERIVATIVE_FEATURE:
        dcurrent_dt = np.gradient(current_model, t).reshape(-1, 1)
        input_features.append(dcurrent_dt)

    X_raw = np.column_stack(input_features)
    Y_raw = np.column_stack([displacement_model, force_model])
    input_feature_names = ["Current_A"]
    if USE_TIME_FEATURE:
        input_feature_names.append("Normalized_Time")
    if USE_CURRENT_DERIVATIVE_FEATURE:
        input_feature_names.append("dCurrent_dt")
    ...
    # Optional final fine tuning on all windows. This improves reconstruction of the
    # only available trajectory. Use the validation metrics before fine tuning as a
    # conservative check, and the final figures as trajectory reproduction results.
    if FINE_TUNE_ON_ALL_WINDOWS:
        print(f"Fine tuning on all available windows for {FINE_TUNE_EPOCHS} epochs...")
        ...
        for ft_epoch in range(1, FINE_TUNE_EPOCHS + 1):
            ...
            for xb, yb in all_loader:
                ...
                torch.nn.utils.clip_grad_norm_(model.parameters(), GRADIENT_THRESHOLD)
                optimizer.step()
            if ft_epoch == 1 or ft_epoch % 100 == 0 or ft_epoch == FINE_TUNE_EPOCHS:
                print(f"Fine tune epoch {ft_epoch:4d} completed")
    ...
    torch.save({
        ...
        "model_config": {
            ...
            "uses_time_feature": USE_TIME_FEATURE,
            "uses_current_derivative_feature": USE_CURRENT_DERIVATIVE_FEATURE,
        },
...
def github_push(source_folder: Path) -> None:
    ...
    # Stash any previous local changes in the temporary repo before pulling.
    run_command(["git", "stash", "push", "-u", "-m", "auto-stash-before-python-lstm-pull"], cwd=repo_folder)
    ...
    if source_fig.is_dir():
        # Do not delete the target figures folder on Windows because OneDrive,
        # antivirus, or Git can temporarily lock files and cause WinError 5.
        # Instead, overwrite and add files safely.
        ...
        for src_file in source_fig.rglob("*"):
            if src_file.is_file():
                ...
                try:
                    ...
                    print(f"Warning: could not overwrite locked file: {dst_file}")
