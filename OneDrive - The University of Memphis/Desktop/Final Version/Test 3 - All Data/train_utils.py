...
The model architecture is unchanged. The improvements are limited to:

1. validation-based learning-rate reduction,
2. validation-based early stopping,
3. moderate emphasis on large displacement responses,
4. restoring the best validation epoch.
"""
...
def accuracy_focused_loss(prediction, target):
    """Ordinary squared error with moderate displacement-peak emphasis.

    Both outputs are already standardized using training-only statistics.
    Therefore, displacement and force start on comparable numerical scales.
    """
    ...
    # Larger displacement responses receive moderately larger importance.
    peak_weight = (
        1.0
        + DISPLACEMENT_PEAK_WEIGHT
        * torch.abs(target[:, 0])
...
def fine_tune_on_all_development_data(model, data, device):
    """Use all 67-127 mA development windows for four final updates.

    Validation has already selected the model and hyperparameters.
    This step only lets the selected model use the validation blocks
    before the untouched 147 mA final test.
    """
