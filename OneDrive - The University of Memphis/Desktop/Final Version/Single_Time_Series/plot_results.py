...
# 3. Measured/predicted test blocks, errors, and parity.
for index, short_name, display_name, unit in OUTPUTS:
    ...
    plt.legend()
    finish_figure(f"03_test_{short_name}_tracking.png")

    error = measured[:, index] - prediction[:, index]
    plt.figure(figsize=(12, 4))
    ...
    finish_figure(f"04_test_{short_name}_error.png")

    minimum = min(measured[:, index].min(), prediction[:, index].min())
    maximum = max(measured[:, index].max(), prediction[:, index].max())
    plt.figure(figsize=(6, 6))
    plt.scatter(measured[:, index], prediction[:, index], s=12, alpha=0.45)
    ...
    plt.legend()
    finish_figure(f"05_test_{short_name}_parity.png")
...
table.scale(1, 1.5)
plt.title(f"{SINGLE_SERIES_SHEET}: distributed test metrics", pad=18)
plt.tight_layout()
plt.savefig(FIGURES_FOLDER / "06_test_metrics.png", dpi=180, bbox_inches="tight")
plt.close()
