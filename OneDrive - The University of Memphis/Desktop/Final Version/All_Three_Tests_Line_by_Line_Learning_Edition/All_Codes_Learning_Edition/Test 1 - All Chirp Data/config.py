...
# Divide the original 2000 Hz rate by four. The resulting 500 Hz rate is
# well above twice the maximum 31 Hz chirp frequency and reduces runtime.
DOWNSAMPLE_FACTOR = 4
...
# A stride of two keeps every second training target to reduce duplicate
# windows. Validation and tests use stride one for dense, fair evaluation.
TRAIN_STRIDE = 2
# A validation stride of one scores every possible target window.
VALIDATION_STRIDE = 1
# A test stride of one also scores every possible target window.
TEST_STRIDE = 1
