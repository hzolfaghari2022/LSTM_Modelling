...
"""Run the selectable one-series deep LSTM simulation."""
...
# 1. Load one chirp and distribute its train, validation, and test blocks

# Store the prepared data or the table returned by the current read operation.
data = prepare_data(HERE)
# Select data["split_table"].to_csv(RESULTS_FOLDER / "data_split.csv", index=False) from the prepared dataset for this operation.
...
# 2. Build the same 3-32-64-64-2 model used in Test 1
...
# 3. Evaluate held-out blocks and predict the full record for visualization
...
# Use the expression `test_measured, test_prediction = evaluate(` as the next part of the surrounding Python statement.
test_measured, test_prediction = evaluate(
    # Select `data["x_test"]` from the current array, tensor, table, or dictionary.
    data["x_test"],
    # Select `data["y_test"]` from the current array, tensor, table, or dictionary.
    data["y_test"],
# Close the current function call, tuple, or grouped expression.
)
# Use the expression `all_measured, all_prediction = evaluate(` as the next part of the surrounding Python statement.
all_measured, all_prediction = evaluate(
    # Select `data["x_all"]` from the current array, tensor, table, or dictionary.
    data["x_all"],
    # Select `data["y_all"]` from the current array, tensor, table, or dictionary.
    data["y_all"],
# Close the current function call, tuple, or grouped expression.
)
# Use the expression `raw_time, _, raw_measured = data["experiment"]` as the next part of the surrounding Python statement.
...
evaluation_sets = [
    ...
    (
        ...
        # Pass `test_measured` as the next value required by the surrounding call or collection.
        test_measured,
        # Pass `test_prediction` as the next value required by the surrounding call or collection.
        test_prediction,
    # Close the current function call, tuple, or grouped expression.
    ),
    # Begin the grouped expression or collection continued on the following lines.
    (
        ...
        # Pass `all_measured` as the next value required by the surrounding call or collection.
        all_measured,
        # Pass `all_prediction` as the next value required by the surrounding call or collection.
        all_prediction,
    # Close the current function call, tuple, or grouped expression.
    ),
...
# Held-out rows are the fair result; full-record rows are descriptive.
metric_rows = []
...
for evaluation_name, measured, prediction in evaluation_sets:
    ...
    # Begin the indented block controlled by this statement.
        ...
        row = {
            ...
            # Store the 'Series' field in the current dictionary.
...
save_predictions(
    ...
    # Select `data["test_time"]` from the current array, tensor, table, or dictionary.
    data["test_time"],
    # Pass `test_measured` as the next value required by the surrounding call or collection.
    test_measured,
    # Pass `test_prediction` as the next value required by the surrounding call or collection.
...
save_predictions(
    ...
    # Select `data["all_time"]` from the current array, tensor, table, or dictionary.
    data["all_time"],
    # Pass `all_measured` as the next value required by the surrounding call or collection.
    all_measured,
    # Pass `all_prediction` as the next value required by the surrounding call or collection.
...
torch.save(
    # Begin the grouped expression or collection continued on the following lines.
    {
        ...
        # Store the 'selected_series' field in the current dictionary.
...
# Generate figures only after all numerical results have been saved.
subprocess.run(
    # Begin the grouped expression or collection continued on the following lines.
