"""Load and partition the five chirp experiments.

The first three records are training-only.  The fourth record uses
interleaved training/validation/test blocks so every role covers the
chirp instead of occupying one continuous frequency interval.  The
fifth record remains an untouched pure test.
"""
...
def load_sheet(workbook: Path, sheet_name: str):
    ...
    # COMSOL exported some repeated time rows; preserve the final value.
    data = (
    ...
    # The chirp ends at 31 Hz, so anti-aliased 500 Hz data preserve it.
...
def make_distributed_blocks(number_of_samples: int):
    ...
    for block_number, start in enumerate(
        ...
        # A block shorter than the sequence history cannot form a window.
        role = (
...
def prepare_data(folder: Path):
    ...
    # Scaling uses only samples whose assigned role is training.
    train_features = []
    train_outputs = []
    split_rows = []
