...
for index, short_name, display_name, unit in OUTPUTS:
    ...
    # Standard distributed test error filename
    plt.figure(figsize=(12, 4))
    ...
    # Pure-test error filename expected by slide setup (06_displacement_error_vs_time & 06_force_error_vs_time)
    plt.figure(figsize=(12, 4))
    plot_discontinuous(test_time, error, color=RED)
    plt.axhline(0, color="black", linestyle="--", linewidth=1)
    plt.xlabel("Time (s)")
    plt.ylabel(f"{display_name} error ({unit})")
    plt.title(f"Pure-test {display_name} prediction error")
    finish_figure(f"06_{short_name}_error_vs_time.png")
...
axis.text(
    ...
    "*Complete-record rows are descriptive because they include fitted regions.",
