...
metric_ranges = [
    np.ptp(test_measured[:, 0]),
    np.ptp(test_measured[:, 1]),
    np.ptp(all_measured[:, 0]),
    np.ptp(all_measured[:, 1]),
]
metric_samples = [len(test_time), len(test_time), len(all_time), len(all_time)]

display_metrics = metrics.drop(columns=["Series"]).copy()
display_metrics["Evaluation"] = display_metrics["Evaluation"].replace(
    {
        "Distributed held-out test blocks": "Internal held-out\n127 mA",
        "Complete selected record (descriptive)": "Complete record*\n127 mA",
    }
)
...
axis.axis("off")
axis.set_title("Test 2 — held-out and complete-record performance", fontsize=16, pad=18, weight="bold")
table = axis.table(
    cellText=table_rows,
...
axis.text(
    ...
    "Internal held-out 127 mA: distributed blocks from the chirp that were not used to fit the model.\n"
    "Complete record 127 mA*: prediction over the entire record, including fitted regions; this is descriptive, not an independent test.\n"
    "Metric direction: higher R² and Fit are better; lower MSE, RMSE, MAE, and NRMSE are better.",
    transform=axis.transAxes,
    ha="left",
    va="bottom",
    fontsize=9.5,
    color=GRAY,
    bbox={"boxstyle": "round,pad=0.65", "facecolor": "#eef5f9", "edgecolor": "#90b6cf"},
)
figure.tight_layout()
