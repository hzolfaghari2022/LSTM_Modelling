...
# Exact feature order used by the primary one-step displacement LSTM.  These
# are causal model features, not thirteen independently applied actuator
# inputs.  The only externally applied input is coil current.
...
class OneStepDisplacementLSTM(nn.Module):
    ...
    def __init__(self, input_features=len(ONE_STEP_FEATURE_NAMES)):
        super().__init__()
...
class _OneStepDataset(Dataset):
    def __init__(self, records, pairs, feature_mean, feature_std,
        ...
        self.feature_indices = np.asarray(
            range(len(ONE_STEP_FEATURE_NAMES))
            if feature_indices is None else feature_indices,
            dtype=np.int64,
        )
...
def run_one_step_study(
    ...
    if active_feature_indices.ndim != 1 or len(active_feature_indices) < 1:
        raise ValueError("At least one one-step feature must be retained.")
    if len(set(active_feature_indices.tolist())) != len(active_feature_indices):
        raise ValueError("feature_indices contains duplicate entries.")
    if np.any(active_feature_indices < 0) or np.any(
        active_feature_indices >= len(ONE_STEP_FEATURE_NAMES)
    ):
        raise ValueError("feature_indices contains an invalid feature index.")

    active_feature_names = [
        ONE_STEP_FEATURE_NAMES[index] for index in active_feature_indices
    ]
    print(
        f"Retained LSTM features ({len(active_feature_names)}/"
        f"{len(ONE_STEP_FEATURE_NAMES)}): " + ", ".join(active_feature_names)
    )
    ...
    statistics = _training_statistics(
        records, training_pairs, active_feature_indices
    )
    feature_mean, feature_std, target_mean, target_std = statistics
    ...
    model = OneStepDisplacementLSTM(
        input_features=len(active_feature_indices)
    ).to(device)
    ...
    torch.save(
        {
            ...
            "all_available_feature_names": list(ONE_STEP_FEATURE_NAMES),
