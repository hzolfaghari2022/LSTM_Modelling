...
from config import (
    ...
    REFINEMENT_SAMPLES_PER_RECORD,
    REFINE_PHYSICAL_MODEL,
    USE_GREY_BOX,
    USE_STATIC_BASELINE,
    VALIDATION_STRIDE,
    DEVELOPMENT_WORKBOOK_NAME,
    INDEPENDENT_TEST_WORKBOOK_NAME,
)
import grey_box
...
def find_workbook(folder: Path) -> Path:
    """
    Return only the exact development workbook beside main.py.

    No wildcard, parent-folder search, newest-file rule, or environment
    override is used.  This prevents similarly named Excel files from being
    included accidentally.
    """
    folder = Path(folder).resolve()
    chosen = folder / DEVELOPMENT_WORKBOOK_NAME
    if chosen.is_file():
        return chosen
    ...
    lines = [
        f"Required workbook not found: {chosen}",
        "",
        f"Place a file named exactly {DEVELOPMENT_WORKBOOK_NAME} beside main.py.",
    ]
    if nearby:
        lines.append("")
        lines.append("Other spreadsheets present in that folder:")
        for name in nearby:
            lines.append(f"  {name}")
    raise FileNotFoundError("\n".join(lines))


def find_independent_test_workbook(folder: Path):
    """Return the exact optional test workbook, or ``None`` when absent."""
    candidate = Path(folder).resolve() / INDEPENDENT_TEST_WORKBOOK_NAME
...
def read_sheet_frame(workbook: Path, sheet_name: str, cache: dict):
    ...
    if sheet_name not in cache:
        source = cache.get("__excel_file__", workbook)
...
def load_record(workbook: Path, specification: dict, cache: dict) -> dict:
    ...
    # Prepend the measured state that the simulation started from.
    #
    ...
    # coil current at its initial value, so repeating the measured first state
    # backwards supplies causal history without using any future output.
    ...
    first = pad
    velocity0 = (
        float(outputs[first + 1, 0]) - float(outputs[first, 0])
    ) / TARGET_TIME_STEP
    record["initial_state"] = np.asarray(
        [outputs[first, 0], velocity0, outputs[first, 1]], dtype=np.float32
    )
    return record
...
def make_blocks(number_of_samples, first_sample=0):
    ...
    for block_number, start in enumerate(
        ...
        if (stop - start) < MINIMUM_BLOCK_SAMPLES and blocks:
            # Keep every real sample. A tiny final remainder inherits the
            # preceding block's role instead of being silently discarded.
            previous_number, previous_start, _, previous_role = blocks[-1]
            blocks[-1] = (
                previous_number, previous_start, stop, previous_role
            )
            continue
        role = SPLIT_PATTERN[block_number % len(SPLIT_PATTERN)]
...
def record_window_indices(records, record_index, stride=PURE_TEST_STRIDE):
    ...
    Thanks to the initial-state padding this starts at the very first simulated
...
def select_pure_test_records(records):
    ...
    """
    if PURE_TEST_RECORD_OVERRIDE:
        available = {record["name"] for record in records}
        if PURE_TEST_RECORD_OVERRIDE not in available:
            choices = "\n  ".join(sorted(available))
            raise ValueError(
                "DLSTM_PURE_TEST_RECORD does not match a discovered record. "
                f"Requested: {PURE_TEST_RECORD_OVERRIDE}\nAvailable:\n  {choices}"
            )
        return [PURE_TEST_RECORD_OVERRIDE]
    ...
    for rule in PURE_TEST_RULES:
        if rule == "heaviest_load_unseen_excitations":
            # Keep the heaviest mass represented in training, but hold out two
            # of its four excitations. The test then asks whether the model
            # generalises to a new input signal at a known mass, instead of to
            # a mass 73 percent outside anything it has seen.
            if len(masses) > 1:
                selected += [
                    record["name"]
                    for record in records
                    if record["load_mass_g"] == heaviest
                    and record["family"] in ("step", "dc_plus_sine")
                ]

        elif rule == "heaviest_load":
            if len(masses) > 1:
                selected += [
                    record["name"]
                    for record in records
...
def prepare_data(folder: Path):
    """Load development data plus an optional untouched independent test."""
    workbook = find_workbook(folder)
    with pd.ExcelFile(workbook, engine="openpyxl") as development_excel:
        cache = {"__excel_file__": development_excel}
        discovered = scan_workbook(workbook, excel_file=development_excel)
    ...
    # Choose the built-in pure tests from Total_Data.xlsx before reading the
    # optional workbook.  Independent cases must never alter which development
    # records are held out, even if they introduce a new mass or current range.
    built_in_pure_test_names = select_pure_test_records(records)
    for record in records:
        record["source_workbook"] = workbook.name
        record["is_independent_test"] = False

    # Every case in Test_idpd.xlsx is appended as a complete untouched test.
    # It is intentionally loaded only when the exact file exists beside
    # main.py; there is no wildcard or directory search.
    independent_workbook = find_independent_test_workbook(folder)
    independent_test_names = []
    if independent_workbook is not None:
        with pd.ExcelFile(independent_workbook, engine="openpyxl") as independent_excel:
            independent_cache = {"__excel_file__": independent_excel}
            independent_discovered = scan_workbook(
                independent_workbook, excel_file=independent_excel
            )
            independent_records = [
                load_record(independent_workbook, entry, independent_cache)
                for entry in independent_discovered
            ]
        ...
        independent_base_names = name_records(
            independent_discovered, independent_measurements
        )
        occupied_names = {record["name"] for record in records}
        for position, (record, base_name) in enumerate(
            zip(independent_records, independent_base_names), start=1
        ):
            # An optional workbook may contain only one mass, so its local
            # Load1/Load2 rank would be misleading.  Put the actual load mass
            # in the exported name instead.
            family_part = base_name.split("_", 1)[-1]
            mass_part = f"{record['load_mass_g']:g}".replace(".", "p")
            name = f"Independent_Mass{mass_part}g_{family_part}"
            if name in occupied_names:
                name = f"{name}_record{position}"
            occupied_names.add(name)
            record["name"] = name
            record["source_workbook"] = independent_workbook.name
            record["is_independent_test"] = True
            independent_test_names.append(name)
        records.extend(independent_records)

    pure_test_names = built_in_pure_test_names + independent_test_names
    for record in records:
        record["is_pure_test"] = record["name"] in pure_test_names

    # Keep duplicate reporting honest across both workbooks.  Independent
    # records are never dropped: if one duplicates development data, the
    # report exposes that fact instead of silently changing the user's file.
    duplicate_groups = find_duplicate_records(records)

    # A new optional "independent" record that is identical to a development
    # record is data leakage, not independent evidence.  Fail safely instead
    # of reporting an artificially strong pure-test result.
    role_of = {
        record["name"]: ("pure_test" if record["is_pure_test"] else "development")
        for record in records
    }
    leaking_duplicate_groups = [
        group for group in duplicate_groups
        if {role_of[name] for name in group} == {"development", "pure_test"}
    ]
    if leaking_duplicate_groups:
        details = "\n".join("  " + " == ".join(group) for group in leaking_duplicate_groups)
        raise RuntimeError(
            "Data leakage detected: an untouched test record is identical to "
            "a development record:\n" + details +
            "\nRemove the duplicate from Test_idpd.xlsx before evaluation."
        )
    ...
    if USE_GREY_BOX:
        ...
        if REFINE_PHYSICAL_MODEL:
            physical_model = grey_box.refine_by_simulation(
                development_records,
                training_masks,
                physical_model,
                TARGET_TIME_STEP,
                max_evaluations=REFINEMENT_EVALUATIONS,
                max_samples_per_record=REFINEMENT_SAMPLES_PER_RECORD,
            )
        for record in records:
    ...
    return {
        "workbook": workbook,
        "independent_test_workbook": independent_workbook,
        "records": records,
        "physical_model": physical_model,
        "pure_test_names": pure_test_names,
        "built_in_pure_test_names": built_in_pure_test_names,
        "independent_test_names": independent_test_names,
...
def make_synthetic_record(name, load_mass_g, current, duration_seconds,
    ...
    if initial_state.shape != (3,):
        raise ValueError("initial_state must contain x0, velocity0, and force0.")
    outputs = np.zeros((samples, 2), dtype=np.float32)
    outputs[:, 0] = initial_state[0]
    outputs[:, 1] = initial_state[2]
    outputs[pad, 0] = initial_state[0]
    if pad + 1 < samples:
        outputs[pad + 1, 0] = (
            initial_state[0] + TARGET_TIME_STEP * initial_state[1]
        )
