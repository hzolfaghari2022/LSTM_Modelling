...
# Exact channels used by the active one-step displacement LSTM.  These are
# intentionally separate from config.FEATURE_NAMES, which belong to the
# older autonomous two-output model.
...
def _make_causal_arrays(record):
    ...
    if features.shape[1] != len(ONE_STEP_FEATURE_NAMES):
        raise RuntimeError("One-step feature matrix does not match its names.")
...
def run_one_step_study(data, records, device, results_folder, figures_folder):
    ...
    def evaluate(name, kind, pairs):
        ...
        # Short numbered filenames avoid Windows' legacy 260-character path
        # limit. The map preserves the complete role and experiment name.
        prediction_file_name = f"prediction_{len(prediction_file_rows) + 1:03d}.csv"
        pd.DataFrame(
            {
                "time_s": time,
        ...
        ).to_csv(prediction_folder / prediction_file_name, index=False)
        prediction_file_rows.append(
            {
                "File": prediction_file_name,
                "Kind": kind,
                "Evaluation": name,
            }
        )
    ...
    pd.DataFrame(prediction_file_rows).to_csv(
        results_folder / "one_step_prediction_file_map.csv", index=False
    )
