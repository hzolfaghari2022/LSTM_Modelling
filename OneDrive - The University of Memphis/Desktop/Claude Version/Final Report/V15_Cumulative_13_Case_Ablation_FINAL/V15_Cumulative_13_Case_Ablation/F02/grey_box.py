...
with measured x(0), x'(0), and F(0). Simulating that equation reproduces the ring
...
def masked_state_derivatives(displacement_m, time_step, selected):
    """Differentiate each training block without reading held-out neighbours."""
    selected = np.asarray(selected, dtype=bool)
    velocity = np.full(len(displacement_m), np.nan, dtype=np.float64)
    acceleration = np.full(len(displacement_m), np.nan, dtype=np.float64)
    changes = np.flatnonzero(
        np.diff(np.concatenate(([False], selected, [False])).astype(np.int8))
    )
    for start, stop in changes.reshape(-1, 2):
        block_velocity, block_acceleration = state_derivatives(
            displacement_m[start:stop], time_step
        )
        velocity[start:stop] = block_velocity
        acceleration[start:stop] = block_acceleration
    return velocity, acceleration
...
def identify(records, training_masks, time_step):
    ...
    for record, mask in zip(records, training_masks):
        ...
        selected = mask[pad:]
        velocity, acceleration = masked_state_derivatives(
            displacement, time_step, selected
        )
...
def simulate(record, parameters, time_step):
    ...
    is held at the selected measured initial state there.
    """
    ...
    initial_state = np.asarray(
        record.get("initial_state", (0.0, 0.0, 0.0)), dtype=np.float64
    )
    position = float(initial_state[0]) / MM_PER_M
    speed = float(initial_state[1]) / MM_PER_M

    # Padding is input-side history only. It contains no measured future
    # samples and carries just the selected initial state.
    displacement[:pad] = position * MM_PER_M
    velocity[:pad] = speed * MM_PER_M
    ...
    for index in range(pad, total):
        ...
        if index == pad:
            force[index] = float(initial_state[2])
        else:
            force[index] = (
...
def simulate_fast(record, parameters, time_step):
    ...
    # Keep the optimiser's fast simulation exactly consistent with
    # simulate().  Resetting these states to zero makes the optimiser explain
    # an artificial initial-condition error by corrupting the spring and
    # damping coefficients, especially on the short step/zero records.
    initial_state = np.asarray(
        record.get("initial_state", (0.0, 0.0, 0.0)), dtype=np.float64
    )
    position = float(initial_state[0]) / MM_PER_M
    speed = float(initial_state[1]) / MM_PER_M
    output[:pad] = position * MM_PER_M
    half = 0.5 * time_step
