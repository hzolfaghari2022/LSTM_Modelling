...
# Exact causal feature order used by the one-step displacement-residual LSTM.
# Incremental ablation case N retains the first N entries in this list.
...
class _OneStepDataset(Dataset):
    def __init__(self, records, pairs, feature_mean, feature_std,
                 target_mean, target_std, feature_indices=None):
        ...
        self.feature_indices = np.asarray(
            ...
            else np.arange(len(ONE_STEP_FEATURE_NAMES)),
            dtype=np.int64,
        )
...
def _training_statistics(records, pairs, feature_indices=None):
    ...
    if feature_indices is not None:
        features = features[:, np.asarray(feature_indices, dtype=np.int64)]
...
def run_one_step_study(
    ...
    if feature_indices is None:
        feature_indices = tuple(range(len(ONE_STEP_FEATURE_NAMES)))
    else:
        feature_indices = tuple(int(index) for index in feature_indices)
    if not feature_indices:
        raise ValueError("At least one one-step LSTM feature must be retained.")
    if len(set(feature_indices)) != len(feature_indices):
        raise ValueError("Feature indices must be unique.")
    if min(feature_indices) < 0 or max(feature_indices) >= len(ONE_STEP_FEATURE_NAMES):
        raise ValueError("Feature index is outside the 13-feature definition.")
    active_feature_names = [ONE_STEP_FEATURE_NAMES[index] for index in feature_indices]
    print(f"LSTM input features ({len(feature_indices)}):")
    for number, name in enumerate(active_feature_names, start=1):
    ...
    statistics = _training_statistics(records, training_pairs, feature_indices)
    feature_mean, feature_std, target_mean, target_std = statistics
    ...
    model = OneStepDisplacementLSTM(input_features=len(feature_indices)).to(device)
    trainable_parameters = sum(
        parameter.numel() for parameter in model.parameters()
        if parameter.requires_grad
    )
    print(
        f"AUDIT: LSTM input size={len(feature_indices)}; "
    ...
    validation_selected_trust = residual_trust
    if residual_trust_override is not None:
        residual_trust = float(residual_trust_override)
        if not 0.0 <= residual_trust <= 1.0:
            raise ValueError("residual_trust_override must be between 0 and 1.")
    print(
        "Validation-selected displacement residual trust: "
        f"{validation_selected_trust:.3f}; trust used: {residual_trust:.3f}"
    )
        ...
        pd.DataFrame(
            {
                ...
                "displacement_baseline_mm": displacement_baseline,
                "lstm_correction_mm": predicted[:, 0] - displacement_baseline,
    ...
    torch.save(
        {
            ...
            "validation_selected_displacement_residual_trust": validation_selected_trust,
            "feature_indices": feature_indices,
            "feature_names": active_feature_names,
            "input_feature_count": len(feature_indices),
