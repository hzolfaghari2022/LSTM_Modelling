...
Every time-history figure displays the first 50% of the available simulation
duration (for example, 0.0--0.8 s for a 1.6 s record).  This is a display-only
limit: model predictions and all reported metrics still use the complete
record.  Labels use 16-point type, ticks and legends use 13-point type, and
legends are automatically placed where they obscure the least data, with an
opaque white background.  Garamond is preferred when installed; otherwise the
code uses Times New Roman, followed by a portable serif fallback.  These shared
settings are in `figure_style.py` and apply to both the one-step and autonomous
figure reports.
