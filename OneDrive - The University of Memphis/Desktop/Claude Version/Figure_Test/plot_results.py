...
from figure_style import configure_plot_style, prepare_figure_for_export
...
def _finish(figure, folder, file_name):
    ...
    prepare_figure_for_export(figure)
...
def _load_saved_report(results_folder):
    ...
    for row in inventory.itertuples(index=False):
        ...
        pieces = []
        for kind in kinds:
            path = prediction_folder / f"{kind}__{row.record}.csv"
            if not path.exists():
