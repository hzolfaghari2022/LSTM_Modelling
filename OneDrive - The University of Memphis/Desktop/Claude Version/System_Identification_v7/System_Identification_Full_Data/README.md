...
Recording these so they are not tried again.

**Refitting the physics against simulated trajectories.** Equation error
differentiates the measured displacement twice, so it is biased, and refitting
against simulated trajectories ought to beat it. It cuts training cost from
0.222 to 0.101 and improves every reference mass test record, then destroys the
heaviest load (0.037 to 0.351 mm on zero input). Leave one mass out confirms
it: equation error transfers at 0.0242 mm, every refined variant at 0.066 to
0.093 mm. With a nine degree spring and two training masses, the extra freedom
is spent contorting the curve where the training data sits. Off by default, see
`config.REFINE_PHYSICAL_MODEL`.

**A richer force law.** Raising the force constant polynomial from degree 3 to
7 moved the worst operating point error only from 3.12 to 2.47 percent, and
current dependent terms did not help at all, so the coil is not saturating at
0.2 to 0.4 A. The model is not short of flexibility.

## Judging accuracy honestly

Use `leave_one_mass_out.py` before adopting any change that touches the physics.
It fits without the 1.906 g records and predicts them, which is the only
available measure of cross mass transfer that does not touch the pure tests.
Training fit alone will mislead you: every rejected idea above improved it.

Judge records on absolute RMSE, not percentage. `Load3_DCSine` reads 17.3
percent but its 0.137 mm is better than `Load3_Step` at 0.212 mm; it only looks
worse because at 7.625 g the 0.2 A current nearly cancels gravity and the swing
is just 0.79 mm.
