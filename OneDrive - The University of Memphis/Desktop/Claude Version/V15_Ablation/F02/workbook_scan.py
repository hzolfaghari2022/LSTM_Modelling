...
def scan_workbook(workbook, sheet_cache=None):
    """
    ...
    # Open the .xlsx archive only once. The earlier implementation passed the
    # workbook path to pd.read_excel inside this loop, which made openpyxl
    # reopen and reparse the complete ZIP/XML workbook for every worksheet.
    # That is especially slow in a OneDrive-synchronised folder.
    with pd.ExcelFile(workbook, engine="openpyxl") as excel:
        sheet_names = list(excel.sheet_names)
        for position, sheet_name in enumerate(sheet_names, start=1):
            print(
                f"  Reading worksheet {position}/{len(sheet_names)}: "
                f"{sheet_name}",
                flush=True,
            )

            if sheet_cache is not None and sheet_name in sheet_cache:
                frame = sheet_cache[sheet_name]
            elif sheet_cache is not None:
                # Cache the complete sheet. load_record() will reuse this
                # DataFrame, so every worksheet is parsed exactly once.
                frame = excel.parse(sheet_name=sheet_name, header=None)
                sheet_cache[sheet_name] = frame
            else:
                # Lightweight metadata-only mode for callers that do not need
                # the numerical rows afterward.
                frame = excel.parse(
            ...
            header = frame.iloc[:METADATA_ROWS]
            if header.empty:
                continue
            ...
            coil_mass = _find_number(
                r"coil\s*mass\s*=\s*([\d.]+)", metadata_text
            )
            load_mass = _find_number(
                r"load\s*mass\s*=\s*([\d.]+)", metadata_text
            )
            ...
            description_row = (
                header.iloc[15] if len(header) > 15 else header.iloc[0]
            )
            sheet_description = ""
            for column in range(header.shape[1]):
            ...
            for start in start_columns:
                description = (
                    _clean(description_row.iloc[start]) or sheet_description
                )
                discovered.append(
                    {
                        "sheet": sheet_name,
