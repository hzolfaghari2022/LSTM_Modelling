...
def finish(figure, file_name):
    figure.savefig(FIGURES_FOLDER / file_name, dpi=170, bbox_inches="tight")
    plt.close(figure)
...
figure.suptitle(
    "Figure 1  Complete data inventory, all 19 COMSOL records "
...
for figure_index, (column, label, unit, color) in enumerate(
    ...
    for position, name in enumerate(internal_names):
        ...
        plot_with_gaps(
            ...
            color=color, linewidth=1.0, linestyle="--", label="LSTM model",
        )
...
if step_names:
    ...
    for position, name in enumerate(step_names):
        ...
        for row, (column, label, unit, color) in enumerate(
            ...
            axis.plot(result["time"], result["predicted"][:, column],
                      color=color, linewidth=1.2, linestyle="--", label="LSTM model")
            r2 = metric_value(name, label, "R2")
    ...
    figure.suptitle(
        "Figure 9  Step input validation. The step family was held out at the "
        "reference mass, so this tests both the transient shape and the "
        "settled value.",
        fontsize=13,
    )
    figure.tight_layout()
...
if zero_names:
    ...
    figure.suptitle(
        ...
        "zero, so the model can only get this right from the configuration "
        "channels.",
        fontsize=13,
    )
    figure.tight_layout()
...
if chirp_name is not None:
    ...
    axes[0][0].plot(time[zoom], result["predicted"][zoom, 0], color=BLUE,
                    linewidth=1.1, linestyle="--", label="LSTM model")
...
if probe_names:
    ...
    figure.suptitle(
        "Figure 12  Model response to two signals that were never simulated. "
        "There is no reference here, this is a physical plausibility check: "
        "the force must follow Bl times current and the displacement must "
        "settle on the gravity sag of each mass.",
        fontsize=12,
    )
    figure.tight_layout()
