...
def find_workbook(folder: Path) -> Path:
    """
    Return the exact shared development workbook.

    The all-model runner supplies DLSTM_TOTAL_DATA to every child process.
    A directly run case still uses the one workbook at the project root.
    Wildcards and newest-file selection are intentionally not used.
    """
    folder = Path(folder).resolve()
    configured = os.environ.get("DLSTM_TOTAL_DATA", "").strip()
    chosen = (
        Path(configured).expanduser().resolve()
        if configured
        else folder / DEVELOPMENT_WORKBOOK_NAME
    )
    if chosen.is_file():
        return chosen
    ...
    lines = [
        ...
        "",
        f"Place a file named exactly {DEVELOPMENT_WORKBOOK_NAME} beside the "
        "top-level main.py, or run the top-level command with --data PATH.",
    ]
    if nearby:
        lines.append("")
...
def find_independent_test_workbook(folder: Path):
    ...
    configured = os.environ.get("DLSTM_INDEPENDENT_DATA", "").strip()
    candidate = (
        Path(configured).expanduser().resolve()
        if configured
        else Path(folder).resolve() / INDEPENDENT_TEST_WORKBOOK_NAME
    )
...
def prepare_data(folder: Path):
    ...
    if independent_workbook is not None:
        independent_cache = {}
        independent_discovered = scan_workbook(independent_workbook)
        independent_records = [
