...
workbook must be stored once at the top level to avoid 13 unnecessary copies of the
...
The measured workbook is not duplicated inside this code package. Before the
first run, either copy your original, unchanged `Total_Data.xlsx` once beside
the top-level `main.py`, or provide its existing full path with `--data`.
...
If the shared workbook is stored elsewhere, set its path before running one
folder directly:

```powershell
$env:DLSTM_TOTAL_DATA="C:\full\path\Total_Data.xlsx"
python main.py
```
...
If the workbook is stored elsewhere, keep it there and run:

```powershell
python main.py --data "C:\full\path\Total_Data.xlsx"
```

You can verify the path without starting any training:

```powershell
python main.py --data "C:\full\path\Total_Data.xlsx" --check-data
```

The path is passed to all 13 child models automatically. Do not copy the
workbook into every model folder.
